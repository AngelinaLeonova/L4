...
public:
	...
	Director(string s1 = "", string s2 = "", string s3 = "", string s4 = "", string spezial = "")
		: name(s1), secondName(s3), Otch(s2), position(s4), Admin(spezial)
	{
		cout << " Конструктр работает" << endl;
	}
	void getDirector() const { cout << "Должность: " << position << endl << "Фамилия: " << secondName << endl << "Имя: " <<  name << endl << "Отчество: " <<  Otch << endl << "Специализация: " << spezial << endl; }
};
...
int main()
{
	...
	Director Uspenskiy("Елена","Михайловна", "Ковалева", "Директор", b);
